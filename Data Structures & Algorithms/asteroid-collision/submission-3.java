class Solution {
    public int[] asteroidCollision(int[] asteroids) {
        ...
        {
            if(!asteroidStack.empty() && asteroid < 0)
            {
                while(!asteroidStack.empty() && asteroidStack.peek() > 0 && Math.abs(asteroid) > asteroidStack.peek()) asteroidStack.pop();

                if(!asteroidStack.empty() && !asteroidStack.empty() && asteroidStack.peek() > 0 && Math.abs(asteroid) == asteroidStack.peek())
                {
                    asteroidStack.pop();
                    continue;
                }
                if(!asteroidStack.empty() && asteroidStack.peek() > 0 && Math.abs(asteroid) < asteroidStack.peek()) continue;
